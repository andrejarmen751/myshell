...
#
# IPv6
inet6_yes=$(ifconfig -a | grep -i inet6 | head -1 | awk '{ printf $1}')
inet6="inet6"
if [ "$inet6_yes" = "$inet6" ]; then
	yellow
	echo "inet6 found"
	nocolor
else
...
#
# bash-completion
check_bash_completion=$(dpkg -l bash-completion 2>/dev/null | grep -i "ii" | awk '{ printf $2}')
bash_completion="bash-completion"
if [ "$check_bash_completion" = "$bash_completion" ]; then
	green
...
# brew
check_brew=$(basename "$(which brew)")
brew="brew"
if [ "$check_brew" = "$brew" ]; then
	green
	echo "brew found"
	nocolor
else
...
#
# python-tk
check_python_tk=$(brew list | grep python-tk | awk -F@ '{print $1}' 2>/dev/null)
python_tk="python-tk"
if [ "$check_python_tk" = "$python_tk" ]; then
	green
...
#
# pyenv
check_pyenv=$(brew list | grep -m 1 -o pyenv 2>/dev/null)
pyenv="pyenv"
if [ "$check_pyenv" = "$pyenv" ]; then
	green
	echo "pyenv found"
	nocolor
else
...
# pyenv-virtualenv
check_pyenv_virtualenv=$(brew list | grep pyenv-virtualenv 2>/dev/null)
pyenv_virtualenv="pyenv-virtualenv"
if [ "$check_pyenv_virtualenv" = "$pyenv_virtualenv" ]; then
	green
